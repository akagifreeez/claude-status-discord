...
- 実行: **k8s CronJob（5分ごと・確実）** — [`k8s/cronjob.yaml`](k8s/cronjob.yaml)。GitHub Actions は手動実行（`workflow_dispatch`）のみ
...
> **フォーラムチャンネルに通知する場合**は環境変数 `DISCORD_FORUM=1` を設定してください。すべての通知を **単一のフォーラム投稿（スレッド）にまとめて** 送ります。初回に 1 つだけスレッドを作成し（タイトルは `DISCORD_FORUM_THREAD_NAME`・省略時 `Claude Status`）、以降のすべての障害・更新はその同じスレッドへ追記します（作成したスレッド ID を `state.json` に保存して再利用。スレッドが削除されていた場合は自動で作り直します）。通常のテキストチャンネルなら不要です。
